...
def get_pos_feature(train_data):
    ...
    word_sentence_list = ws(train_data)
    pos_sentence_list = pos(word_sentence_list)
    # print(word_sentence_list)
    del ws
    del pos
    ...
    pos_feature = []
    for i in range(len(pos_sentence_list)):
        dialogue_pos = pos_sentence_list[i]
        pos_feature.append([])
        for j in range(len(dialogue_pos)):
            pos = dialogue_pos[j]
            word = word_sentence_list[i][j]
...
if __name__ == '__main__':
    ...
    # Load data.
    # file_path = './train_2.txt' #SampleData_deid.txt
    file_path = './SampleData_deid.txt'
    # file_path = './train_1_update.txt'
...
for test_id in range(len(y_pred)):
    ...
    for pred_id in range(len(y_pred[test_id]) - 1):
        ...
        pos+=1
# %%
output_path='output_fasttext_1213.tsv'
with open(output_path,'w',encoding='utf-8') as f:
    f.write(output)
print(output)
